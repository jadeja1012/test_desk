...
class sale_order(models.Model):
    ...
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True, readonly=True, default=_get_default_shop)
...
class Pricelist(models.Model):
    ...
    hotel_pricelist = fields.Boolean(string="Hotel Pricelist")

class PricelistItem(models.Model):
    _inherit = "product.pricelist.item"

    @api.constrains('product_tmpl_id')
    def _check_duplicate_pricelist(self):
        for record in self:
            if not record.product_tmpl_id or not record.date_start or not record.pricelist_id:
                continue  # avoid false positives for incomplete records
            ...
            if duplicates:
                raise ValidationError(
                    "A pricelist item already exists for this product on the same date in this pricelist."
                )
