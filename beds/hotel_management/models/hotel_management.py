...
class id_master(models.Model):
    ...
    name = fields.Char('ID Name', required=True,tracking=True)
    id_code = fields.Char('ID Code', required=True,tracking=True)
...
class checkout_configuration(models.Model):
    ...
    name = fields.Selection([('custom', 'Custom'), ('24hour', '24 Hours')],
                            'Checkout Time', default='24hour', required=True,tracking=True)
    ...
    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Company', store=True,tracking=True)
...
class hotel_reservation(models.Model):
    ...
    new_currency_id = fields.Many2one('res.currency', string='Currency Reservation')
    invoice_count = fields.Integer(compute="_compute_invoice", string='Invoice Count', copy=False, default=0,
                                   readonly=True)
    complimentary = fields.Selection([
        ('all_meals', 'All Meals'),
        ('no_meals', 'No Meals'),
        ('breakfast_dinner', 'Breakfast & Dinner'),
        ('breakfast', 'Breakfast')
    ], string='Meal Type', tracking=True)
    beds_ref_id = fields.Char("Booking Ref")
    ...
    def send_email(self):
        template = self.env.ref('hotel_management.send_confirm_mail')
                            ...
                            if (history_start_date <= reservation_start_date < history_end_date) or (
                                ...
                                if not (reservation.id == history.booking_id.id):
                                    raise UserError("Room  %s is booked in this reservation period !" % (
                                        room_line_id.name,))
            for line in reservation.reservation_line:
                if line.room_number.name != 'All Meals' or line.room_number.name != 'Breakfast' or line.room_number.name != 'Breakfast And Dinner':
                    room_line_id = self.env['hotel.room'].search(
                        [('product_id', '=', line.room_number.id)])
    ...
    @api.depends('reservation_no', 'reservation_line', 'invoice_count')
    def _compute_invoice(self):
        for reservation in self:
            if reservation.reservation_no:
                invoices = self.env['account.payment'].search([('memo', "=", reservation.reservation_no)])
                reservation.payment_move_ids = invoices
                reservation.invoice_count = len(invoices)
            else:
                reservation.invoice_count = 0
                
    def action_view_payment(self):
        invoices = self.mapped('payment_move_ids')
    ...
    def create(self, vals):
        for val in vals:
            ...
            if res:
                ...
                if res.reservation_line:
                    for line in res.reservation_line:
                        if line and line.room_number:
                            if isinstance(line.checkin, datetime):
                                reservation_checkin = line.checkin.date()
                            else:
                                reservation_checkin = fields.Date.from_string(line.checkin)
                            if isinstance(line.checkout, datetime):
                                reservation_checkout = line.checkout.date() 
                            else:
                                reservation_checkout = fields.Datetime.from_string(line.checkout)

                            repair_records = self.env['rr.housekeeping'].search([
                       ('room_no.product_id', '=', line.room_number.id), 
                        ('state', 'not in', ['done'])
                    ])
                            if repair_records:
                                for repair in repair_records:
                                    repair_date = repair.date.date() 
                                    if reservation_checkin == repair_date:
                                        raise ValidationError(_('The room %s is currently under repair and cannot be reserved during your selected dates.') % line.room_number.name)
    ...
    def update_prices(self):
        ...
        pricelist_amount = self.pricelist_id.currency_id.rate
        self.new_currency_id = self.pricelist_id.currency_id.id
        for line in self.reservation_line:
            pricelist_rule = line.pricelist_item_id
            order_date = line.line_id.date_order or fields.Date.today()
            product = line.room_number
            qty = line.number_of_days or 1.0
            uom = line.room_number.uom_id
            pricev = pricelist_rule._compute_price(product, qty, uom, order_date, currency=self.new_currency_id)
            line.update({
                'price': pricev,
            })
    ...
    def onchange_patner(self):
        if self.partner_id:
            ...
            if self.partner_id.id_line_ids:
                self.id_line_ids = self.partner_id.id_line_ids.ids
            else:
                self.id_line_ids =False

    name = fields.Char('Event type', default=lambda self: _('New'),tracking=True)
    gds_id = fields.Char('GDS ID', readonly=True)
    reservation_no = fields.Char(
        'Reservation No', readonly=True, index=True,tracking=True)
    date_order = fields.Datetime('Date Ordered', required=True, default=datetime.today(), readonly=True,tracking=True)
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True,
                              domain="[('company_id', '=', [current_company_id])]",
                              default=_get_default_shop, readonly=True,tracking=True)
    partner_id = fields.Many2one(
        'res.partner', 'Guest Name', required=True,tracking=True)
    pricelist_id = fields.Many2one(
    'product.pricelist',
    string='Pricelist',
    domain=[('hotel_pricelist', '=', True)],
    tracking=True
)
    ...
    adults = fields.Integer('Adults' ,tracking=True)
    childs = fields.Integer('Children',tracking=True)
    #complimentary = fields.Boolean('Complimentary Breakfast',tracking=True)
    reservation_line = fields.One2many(
        'hotel.reservation.line', 'line_id', 'Reservation Line', readonly=False)
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm'), (
        'done', 'Done'), ('cancel', 'Cancelled')], 'State', default='draft', readonly=True,tracking=True)
    ...
    untaxed_amt = fields.Float(
        compute="_get_subtotal_amount", string="Untaxed Amount",tracking=True)

    total_tax = fields.Float(string="Reservation Tax",store = True,
                             compute="_get_total_tax",tracking=True)
    deposit_cost1 = fields.Float(string="deposit cost", )
    total_cost1 = fields.Float(
        string="Total Reservation cost", compute="_get_total_rental_cost", readonly=True,tracking=True)
    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Company', store=True)

    id_line_ids = fields.Many2many('hotel.resv.id.details',
                                  string='ID Line', readonly=False,domain="[('partner_id', '=',partner_id)]")
    ...
    total_advance = fields.Float(
        compute="_get_advance_payment", string="Total Advance Payment",tracking=True )
    remaining_amt = fields.Float(
        compute="_get_advance_payment", string="Total Remaining Amount",tracking=True )
    ...
    online_advance_amount = fields.Float(string="Online Advance")
    ...
    def confirmed_reservation(self):
        for reservation in self:
            ...
            if not breakfast_added:
                #days = len(reservation.price_days_id_folio)
                unique_nights = set()
                total_nights = 0
                for line in reservation.reservation_line:
                    checkin_date = line.checkin.strftime('%Y-%m-%d')
                    checkout_date = line.checkout.strftime('%Y-%m-%d')
                    ...
                    while current_date < checkout_date:
                        unique_nights.add(current_date)
                        current_date += timedelta(days=1)

                if line:
                    if line.line_id.complimentary == "all_meals" and reservation.source == 'internal_reservation':
                        breakfast = self.env["product.product"].search([("name", "=", "All Meals")])
                        if breakfast:
                            tax_ids = breakfast.taxes_id.ids
                            new_order_line = (0, 0, {
                                'checkin': line.checkin,
                                'checkout': line.checkout,
                                'categ_id': breakfast.categ_id.id,
                                'room_number': breakfast.id,
                                'taxes_id': [(6, 0, tax_ids)],
                                'line_id': reservation.id,
                                'price': breakfast.list_price * reservation.adults * len(unique_nights),
                                'number_of_days': 1,
                            })
                            reservation.write({'reservation_line': [new_order_line]})
                            breakfast_added = True
                            break
                    if line.line_id.complimentary == "breakfast" and reservation.source == 'internal_reservation':
                        breakfast = self.env["product.product"].search([("name", "=", "Breakfast")])
                        if breakfast:
                            tax_ids = breakfast.taxes_id.ids
                            new_order_line = (0, 0, {
                                'checkin': line.checkin,
                            ...
                            breakfast_added = True
                            break
                    if line.line_id.complimentary == "breakfast_dinner" and reservation.source == 'internal_reservation':
                        breakfast = self.env["product.product"].search([("name", "=", "Breakfast And Dinner")])
                        if breakfast:
                            tax_ids = breakfast.taxes_id.ids
                            new_order_line = (0, 0, {
                                'checkin': line.checkin,
            ...
            for line in reservation.reservation_line:
                ...
                #
                ###############################################
                '''overlapping_reservations = self.env['hotel.reservation'].search([
                ('reservation_line.room_number', '=', line.room_number.id),
                ('state', 'in', ['confirm', 'done']), 
                ('id', '!=', reservation.id)])
                print("1111111111111111111111111",overlapping_reservations)

                for res in overlapping_reservations:
                    for res_line in res.reservation_line:
                        res_start_date = datetime.strptime(str(res_line.checkin), '%Y-%m-%d %H:%M:%S').date()
                        res_end_date = datetime.strptime(
                        str(res_line.checkout), '%Y-%m-%d %H:%M:%S').date()
                        reservation_start_date = datetime.strptime(
                        str(line.checkin), '%Y-%m-%d %H:%M:%S').date()
                        reservation_end_date = datetime.strptime(
                        str(line.checkout), '%Y-%m-%d %H:%M:%S').date()

                        if (res_start_date <= reservation_start_date < res_end_date) or (
                            res_start_date < reservation_end_date <= res_end_date) or (
                            (reservation_start_date < res_start_date) and (
                            reservation_end_date >= res_end_date)):
                            print("1111111111111111111111111",res_line.checkin)
                            raise UserError("Room  %s is already booked during this period in another reservation!" % (
                            room_line_id.name))'''
            ...
            total_child = 0 
            for line in reservation.reservation_line:
                total_adult +=line.room_number.max_adult
                total_child +=line.room_number.max_child
                now_time = datetime.now()
                user = self.env.user
                tz = pytz.timezone(user.tz) or pytz.utc
                user_tz_date = pytz.utc.localize(now_time).astimezone(tz)
                user_hour_min = user_tz_date.strftime("%H:%M")
            ...
            if reservation.adults > total_adult:
                raise ValidationError("Need to add more rooms based on the number of guests")
            if reservation.childs > total_child:
                raise ValidationError("Need to add more rooms based on the number of guests") 
    ...
    def create_folio(self):
        for reservation in self:
            ...
            for line in reservation.reservation_line:
                ...
                date_order_user_tz = fields.Datetime.context_timestamp(self, line.checkin).astimezone(pytz.timezone(user_tz))
                hour_minute = date_order_user_tz.strftime("%H:%M")

                line_date = date_order_user_tz
                user = self.env['res.users'].browse([2])
                tz = pytz.timezone(user.tz) or pytz.utc
                now_time = datetime.now()
                user_tz_date = pytz.utc.localize(now_time).astimezone(tz)
                user_hour_min = user_tz_date.strftime("%H:%M")
                user_hour_min_date = user_tz_date
                
                current_datec = date.today()
                if current_datec !=  line.checkin.date():
                    raise ValidationError("Your check-in time does not match the records of your reservation. Please review your booking details.")  
                #if hour_minute  >  user_hour_min:
                    #if not line.early_checkin:
                    #raise ValidationError("Your check-in time does not match the records of your reservation. Please review your booking details.")  
                room_line_id = self.env['hotel.room'].search(
                    [('product_id', '=', line.room_number.id)])
                ###############################################by pornima######
            ...
            folio = self.env['hotel.folio'].create({
                ...
                'id_line_ids':reservation.id_line_ids.ids
            })
            for line in reservation.reservation_line:
                if line.room_number.name != "All Meals" :
                    if line.room_number.name != "Breakfast" :
                        if line.room_number.name != "Breakfast And Dinner" :
                            room_line_id = self.env['hotel.room'].search(
                        [('product_id', '=', line.room_number.id)])
                            room_his_id = self.env['hotel.room.booking.history'].create({
                    'partner_id': reservation.partner_id.id,
                ...
                vals = {
                    ...
                    'product_uom_qty': 1 if line.line_id.shop_id.dynamic_pricelist else line.number_of_days,
...
class hotel_reservation_line(models.Model):
    ...
    @api.depends('room_number', 'number_of_days')
    def _compute_pricelist_item_id(self):
        for line in self:
            if not line.room_number  or not line.line_id.pricelist_id:
                line.pricelist_item_id = False
            else:
                line.pricelist_item_id = line.line_id.pricelist_id._get_product_rule(
                    line.room_number,
                    line.number_of_days or 1.0,
                    date=line.line_id.date_order,
                )
    ...
    def create(self, vals_list):
        for val in vals_list:
            checkin = odoo_fields.Date.from_string(val.get('checkin'))
            checkout = odoo_fields.Date.from_string(val.get('checkout'))

            if checkout and checkin > checkout:
                raise UserError('Error! Departure date must be greater than Arrival Date')

            reservation = self.env["hotel.reservation"].browse(val.get('line_id'))
            product = self.env['product.product'].browse(val['room_number'])
            if product.name == 'Breakfast':
                continue
            if product.name == 'Breakfast And Dinner':
                continue
            if product.name == 'All Meals':
                continue
        
            # If dynamic pricelist is enabled
            if reservation.shop_id.dynamic_pricelist:
                total_price = 0
                pricelist_items = self.env['product.pricelist.item'].search([
                ('product_tmpl_id.product_variant_ids.id', '=', val['room_number']),
                ('date_start', '<=', checkout),
                ('date_end', '>=', checkin),
                ])
                if pricelist_items:
                    ...
                    while current_day < checkout:
                        ...
                        for price_list in pricelist_items:

                            if price_list.pricelist_id.id == reservation.pricelist_id.id:
                                date_start = fDate.from_string(price_list.date_start)
                                date_end = fDate.from_string(price_list.date_end)
                                
                                if date_start <= current_day <= date_end:
                                    # Extract and clean price
                                    price = re.sub(r'[^\d.]', '', price_list.price)
                                    price = price.replace(',', '')
                                    ...
                                    break
                        if not matched:
                            raise UserError(f"Couldn't find a pricelist line matching the date: {current_day.strftime('%Y-%m-%d')}")
                        current_day += timedelta(days=1)

                    # Set the calculated price into the vals dict
                    val['price'] = total_price
                else:
                    raise UserError("Couldn't find a pricelist line matching this Date")
            else:
                # If not dynamic, val['price'] should already be set or handled outside
                pass

        return super(hotel_reservation_line, self).create(vals_list)
    ...
    def count_sub_total1(self):
        for line in self:
            ...
            # Check if dynamic_pricelist is enabled for this reservation's shop
            if line.line_id.shop_id.dynamic_pricelist:
                base_price = line.price  # Already includes all days
            else:
                base_price = line.price * line.number_of_days  # Per day × number of days

            # Apply discount
            price_after_discount = base_price * (1 - (line.discount or 0.0) / 100.0)

            # Calculate taxes
            taxes = line.taxes_id.compute_all(price_after_discount, line.line_id.currency_id,
                                          1,  # quantity = 1 because base_price is already full total
                                          product=line.room_number,
    ...
    checkin = fields.Datetime('Checkin Date', required=True, default=lambda self: datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def _get_display_price(self, product):
        if self.line_id.pricelist_id.discount_policy == 'with_discount':
            return product.with_context(pricelist=self.line_id.pricelist_id.id).lst_price
...
class hotel_room_booking_history(models.Model):
    ...
    name = fields.Char('Product Name', readonly=True, required=False)
...
class hotel_folio(models.Model):
    ...
    'sale_order_id',  # Field in the relationship table for sale orders
    'account_payment_id',  # Field in the relationship table for account payments
    " New Payment Details"  # Optional label for the field in the user interface
    )
    ...
    transport_invoice_ids = fields.Many2many(
        ...
        readonly=True)
    id_line_ids = fields.Many2many(
        'hotel.resv.id.details', string='ID Line')
    ...
    def action_checkout(self):
        folio = self.browse(self._ids)
        folio_line = self.env['hotel.folio'].search([("id","=",self.id)])
        for room in self.room_lines:
            if room.product_id.name == 'All Meals' or room.product_id.name == 'Breakfast' or room.product_id.name == 'Breakfast And Dinner':
                continue
            current_date = date.today()
            sdate = room.checkout_date
            #if current_date == sdate.date():
    ...
    def update_folio_history(self):
        ...
        for folio_line in folio.room_lines:
            ...
            tax_ids = []
            if folio_line.product_id.name != 'All Meals' or folio_line.product_id.name != 'Breakfast' or folio_line.product_id.name != 'Breakfast And Dinner':
                room_line_id = self.env['hotel.room'].search(
...
class hotel_restaurant_tables(models.Model):
    ...
    capacity = fields.Integer('Capacity',tracking=True)
    shop_id = fields.Many2one('sale.shop', 'Hotel',domain="[('company_id', '=', [current_company_id])]",
                              help="Will show list of shop that belongs to allowed companies of logged-in user. \n -Assigning shop name to which this table no belongs to.",tracking=True)
    avl_state = fields.Selection([('available', 'Available'), ('book', 'Booked')], 'Availability Status',
                                 default='available', index=True, required=True,tracking=True)
    state = fields.Selection(selection_add=[('draft', 'Draft'), ('edit', 'Edit'), ('confirmed', 'Confirmed'), (
        'canceled', 'Cancel')], string='State', required=True, readonly=True,
                             ondelete={'draft': 'set default', 'edit': 'set default', 'confirmed': 'set default',
                                       'canceled': 'set default'},tracking=True)
...
class hotel_restaurant_order(models.Model):
    ...
    new_currency_id = fields.Many2one('res.currency', string='Currency Restaurant')
    ...
    def update_prices(self):
        ...
        for line in self.order_list:
            pricelist_rule = line.pricelist_item_id
            order_date = self.o_date or fields.Date.today()
            product = line.product_id.product_id
            qty = line.item_qty or 1.0
    ...
    waiter_name1 = fields.Many2one('res.users', 'Waiter User Name',tracking=True)
    partner_id = fields.Many2one('res.partner', 'Customer', required=True,
                                 help="Will show customer name corresponding to selected room no.",tracking=True)
    room_no = fields.Many2one(
        'hotel.room', 'Room No', domain=domain_change,help="Will show list of currently occupied room no that belongs to selected shop.",tracking=True)
    pricelist_id = fields.Many2one(
    ...
    shop_id = fields.Many2one('sale.shop', 'Hotel', default=_get_default_shop, required=True, readonly=True,
                              help="Will show list of shop that belongs to allowed companies of logged-in user.",tracking=True)

    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Company', store=True,tracking=True)

    state = fields.Selection(selection_add=[('draft', 'Draft'), ('confirm', 'Confirmed'), ('order', 'Order Done'), (
        'done', 'Done'), ('cancel', 'Cancelled')], string='State', index=True, required=True, readonly=True,tracking=True)
    flag = fields.Boolean("Flag", default=False)
    amount_subtotal = fields.Float(
        compute="_sub_total", string='SubTotal', compute_sudo=False,tracking=True)
    amount_tax = fields.Float(compute="_amount_tax", string='Tax',tracking=True)
    amount_total = fields.Float(compute="_total", string='Total',tracking=True)
    amount_untaxed = fields.Float(
        'Untaxed Amount', compute='_amount_untaxed', store=True,tracking=True)
...
class res_partner(models.Model):
    ...
    id_line_ids = fields.One2many('hotel.resv.id.details','partner_id',
                                 readonly=False)
...
class hotel_restaurant_order_list(models.Model):
    ...
    @api.depends('product_id', 'item_qty')
    def _compute_pricelist_item_id(self):
        for line in self:
            if not line.product_id  or not line.o_list.pricelist_id:
                line.pricelist_item_id = False
            else:
                line.pricelist_item_id = line.o_list.pricelist_id._get_product_rule(
                    line.product_id,
                    line.item_qty or 1.0,
                    date=line.o_list.o_date,
                )
    ...
    def _sub_total(self):
        ...
        for line in self:
            ...
            taxes = line.tax_id.compute_all(
                price, None, int(line.item_qty), line.product_id, False)
            if line.o_list:
    ...
    product_id = fields.Many2one('hotel.menucard', 'Item Name',tracking=True)
    product_qty = fields.Integer('quantity',tracking=True)
    states = fields.Boolean('States', default=False,tracking=True)
    previous_qty = fields.Integer('quantities',tracking=True)
    total = fields.Integer('total',tracking=True)
    tax_id = fields.Many2many(
        'account.tax', 'restaurant_order_tax', 'order_line_id', 'tax_id', 'Taxes',tracking=True )
    price_subtotal = fields.Float(
        compute="_sub_total", string='Subtotal',tracking=True)
...
class product_template(models.Model):
    ...
    product_nature = fields.Selection(
        [('kot', 'KOT'), ('bot', 'BOT')], 'Product Nature',tracking=True)
    company_id = fields.Many2one('res.company', string='Company'
                                 , domain="[('id', 'in', [current_company_id])]",tracking=True)
    categ_id = fields.Many2one(
        'product.category', 'Room Category',
...
class hotel_restaurant_kitchen_order_tickets(models.Model):
    ...
    ordernobot = fields.Char('BOT Number', readonly=True,tracking=True)
    product_nature = fields.Selection(
        [('kot', 'KOT'), ('bot', 'BOT')], 'Product Nature',tracking=True)
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True,tracking=True)
...
class hotel_resv_id_details(models.Model):
    ...
    file = fields.Binary(string="ID Proof")
...
class account_invoice(models.Model):
    ...
    def onchange_currency_id(self):
        ...
        if payment_rate_currency_id and payment_rate_currency_id.id != company_currency_id:
            tmp = currency_obj.browse(payment_rate_currency_id.id).rate
...
class AdvancePaymentPolicy(models.Model):
    _name = 'advance.payment.policy'
    _description = 'Advance Payment Policy'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin']

    name = fields.Char(string='Policy Name',tracking=True)
    type = fields.Selection([('direct','Direct Booking'),('website','Website Booking')],string="Booking Type",required=True,tracking=True)
    amount = fields.Float(string="Minimum Amount",required=True,tracking=True)
    shop_id = fields.Many2one('sale.shop',string='Hotel',tracking=True)
