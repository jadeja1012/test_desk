...
class rr_housekeeping(models.Model):
    ...
    name = fields.Char('Req No', readonly=True,tracking=True)
    date = fields.Datetime('Date Ordered', required=True, readonly=True, default=lambda *
                           a: time.strftime('%Y-%m-%d %H:%M:%S'),tracking=True)
    activity = fields.Selection([('repair', 'Repair'), ('replaced', 'Replace')], 'Activity',
                                index=True, required=True, readonly=True,tracking=True)
    requested_by = fields.Many2one('res.partner', 'Requested By User', readonly=True,tracking=True)
    requested_by_partner = fields.Many2one('res.partner', 'Requested By ', readonly=True,tracking=True)
    ...
    room_no = fields.Many2one('hotel.room', 'Room No', required=True, readonly=True,tracking=True)
    approved_by = fields.Char('Approved By',tracking=True)
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirmed', 'Confirmed'), ('assign', 'Assigned'), (
        'done', 'Done'), ('cancel', 'Cancel')], 'State', default="draft", readonly=True, index=True,tracking=True)
    complaint = fields.Char('Complaint', readonly=True,tracking=True)
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True, readonly=True,tracking=True)
    company_id = fields.Many2one('res.company', related='shop_id.company_id', string='Company', store=True,tracking=True)
...
class issue_material_details(models.Model):
    ...
    name = fields.Char('Issue Slip',tracking=True)
    request_id = fields.Many2one('rr.housekeeping', 'Request Number',
                                 required=True, readonly=True,tracking=True)
    repair_ids = fields.One2many('rr.housekeeping.line', 'info_id', 'Product Replacement info', readonly=True,tracking=True)
    complaint = fields.Char('Complaint', readonly=True,tracking=True)
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True, readonly=True,tracking=True)
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm'), (
        'done', 'Done')], 'State', default='draft', readonly=True, index=True,tracking=True)
