...
class agent_commission_invoice(models.Model):
    ...
    name = fields.Char("Agent Commission ID", readonly=True,tracking=True)
    current_date = fields.Date("Date", required=True, readonly=True, default=datetime.now(
    ).strftime('%Y-%m-%d'),tracking=True)
    partner_id = fields.Many2one("res.partner", "Agent", required=True, readonly=True,tracking=True)
    ...
    state = fields.Selection([
        ...
        ('done', 'Done'),
        ('cancel', 'Canceled'),
    ], 'Status', default='draft', readonly=True,tracking=True)

    commission_percentage = fields.Float("Commission %", required=True, readonly=True,tracking=True)
    total_amt = fields.Float(compute="_get_total_amt", string="Total", store=True,tracking=True)
...
class AgentRelation(models.Model):
    ...
    agent_id = fields.Many2one('res.partner',string="Agent Name",domain=[('agent','=',True)],tracking=True)
    shop_ids = fields.Many2many('sale.shop', string="Hotel",tracking=True)
