...
class reservation_detail_report1(models.AbstractModel):
    ...
    def get_data(self, obj):
        ...
        for data_list in res:
            if data_list.room_number.name == 'Breakfast':
                continue
            if data_list.room_number.name == 'Breakfast And Dinner':
                continue
            if data_list.room_number.name == 'All Meals':
                continue
...
class reservation_detail_report4(models.AbstractModel):
    ...
    def get_room1(self, obj):
        ...
        rec = []
        for room in res:
            if room.room_number.name == 'Breakfast':
                continue
            if room.room_number.name == 'Breakfast And Dinner':
                continue
            if room.room_number.name== 'All Meals':
                continue
            if room.line_id.state in ['confirm', 'done']:
                ...
                if present == False:
                    ...
                    room_list_cat.append(room.room_number.categ_id.name)
        result = []
        for room_data, room_category in zip(room_list, room_list_cat):
            count = 0
            for room in rec:
