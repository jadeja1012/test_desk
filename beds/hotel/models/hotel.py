...
class hotel_floor(models.Model):
    ...
    name = fields.Char('Floor Name', required=True, index=True,tracking=True)
    ...
    def _check_unique_name(self):
        for record in self:
            if self.search([('name', '=', record.name), ('id', '!=', record.id)]):
                raise ValidationError("The floor name must be unique.")
...
class product_category(models.Model):
    ...
    room_id = fields.Many2one('sale.shop', string='Hotel')


    @api.model_create_multi
    def create(self, vals_list):
        res = super(product_category, self).create(vals_list)
        for rec in res:
            rec.company_id = self.env.user.company_id
            if self.env.context.get('isroomtype'):
                rec.isroomtype = True
                rec.isparent = True
            # Pass hotel from context or from vals
            if 'room_id' in vals_list[0]:
                rec.room_id = vals_list[0]['room_id']
        return res
...
class hotel_room_type(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        records = super(hotel_room_type, self).create(vals_list)
        for rec in records:
            if rec.room_id:
                rec.cat_id.room_id = rec.room_id
        return records
...
class product_template(models.Model):
    ...
    beds24_room_id = fields.Integer('Beds24')
...
class hotel_folio(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', 'Currency', required=True, related='pricelist_id.currency_id')
    ...
    def action_confirm(self):
        for record in self:
            for line in record.room_lines:
                #if line.product_id.name != "breakfast":
        ...
        for order in self:
            ...
            for pos_order in order.pos_order_ids:
                ...
                for line in pos_order.lines:
                    subtotal += line.price_subtotal
            #subtotal = sum(pos.amount_total for pos in order.pos_order_ids)
            self.write({"amount_untaxed":self.amount_untaxed+subtotal})
            self.write({"amount_tax":self.amount_tax+pos_tax})
            self.write({"amount_total":self.amount_untaxed+self.amount_tax})
        return True
        ...
        for line_obj in self:
            ...
            if line_search:
                ...
                if 'product_uom_qty' in vals:
                    dynamic = line_search.line_id.shop_id.dynamic_pricelist
                    if dynamic:
                        number_of_days = 1
                    else:
                        number_of_days = vals['product_uom_qty']
                    line_search.write({'number_of_days': number_of_days})
