...
# class pos_credit_sale(report_sxw.rml_parse):
class pos_credit_sale(models.AbstractModel):
    _name = 'report.hotel_restaurant_pos.pos_credit_sale'
    _description = 'pos credit sale'
    ...
    def _get_payments(self, form):
        ...
        user_ids = form['user_ids'] or self._get_all_users()
        pos_ids = pos_order_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start'] + ' 00:00:00'),('date_order','<=',form['date_end'] + ' 23:59:59'),('state','in',['draft']),('user_id','in',user_ids)])
        data={}
        if pos_ids:
    ...
    def _get_user_names(self, user_ids):
        ...
        return ', '.join([x.name for x in user_obj.browse(user_ids)])
